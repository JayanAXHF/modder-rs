...
struct ModList {
    filtered_items: Vec<ModListItem>,
    list_items: Vec<ModListItem>,
    state: ListState,
}
...
impl FromIterator<ModListItem> for ModList {
    fn from_iter<I: IntoIterator<Item = ModListItem>>(iter: I) -> Self {
        ...
        match action {
            ...
            Action::Mode(mode) => {
                ...
                if self.enabled {
                    ...
                    self.list.filtered_items = Vec::new();
...
impl From<&ModListItem> for ListItem<'_> {
    fn from(value: &ModListItem) -> Self {
...
impl<'a> ModListItem {
    fn format(&self) -> Line<'a> {
        ...
        Line::from(vec![span, id_span, name_span])
    }
}

async fn get_mods(dir: PathBuf) -> Vec<ModListItem> {
